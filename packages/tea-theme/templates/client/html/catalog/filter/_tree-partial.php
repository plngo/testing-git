		...
		<?php if( $item->getStatus() > 0 ) : ?>
			...
			<div class="cat-item catid-<?= $enc->attr( $item->getId() .
				...
				<div class="item-links row">
					<a class="col-10 cat-link name <?= $_SERVER['REQUEST_URI'] == '/' &&  ($enc->html( $item->getName(), $enc::TRUST ) == 'Home') ? 'active' : ( $this->get( 'path', map() )->has( $item->getId() ) ? ' active' : '' ) ?>"
						href="<?= ($enc->html( $item->getName(), $enc::TRUST ) == 'Home') ? '/' : $enc->attr( $this->url( $item->getTarget() ?: $target, $controller, $action, $params, [], $config ) ) ?>">
